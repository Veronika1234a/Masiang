  ...
  return (
    <div ref={panelRef} className="relative">
      <button
        ...
        className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-[#d8deeb] bg-white text-[#4f5b77] transition-colors hover:bg-[#eef1f8] hover:text-[#25365f]"
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-[#c44444] text-[10px] font-bold text-white">
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 w-[340px] rounded-xl border border-[#e1dce8] bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-[#e1dce8] px-4 py-3">
            ...
            {unreadCount > 0 && (
              <button type="button" onClick={markAllNotificationsRead} className="text-[11px] font-bold text-[#4a6baf] hover:text-[#25365f]">
                Tandai semua dibaca
              </button>
            )}
          </div>
          <div className="max-h-[320px] overflow-y-auto">
            {recent.length > 0 ? recent.map((notif) => (
              <button
                ...
                className={`flex w-full flex-col gap-1 px-4 py-3 text-left transition-colors hover:bg-[#f5f3f7] ${!notif.isRead ? "bg-[#f9f7ff]" : ""}`}
              >
                <div className="flex items-center gap-2">
                  {!notif.isRead && <span className="h-2 w-2 shrink-0 rounded-full bg-[#4a6baf]" />}
